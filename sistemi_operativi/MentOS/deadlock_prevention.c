        ...
        uint32_t **mat_need, size_t n, size_t m)
{
    ...
    while (arr_ne(finish, all_true, n))
    {
        ...
        for (i = 0; i < n && (/**/finish[i]==1UL || /**/arr_g_any(mat_need[i], work, m)); i++);
        if (i == n)
        {
            // Free memory.
            kfree(work);
        ...
        size_t n, size_t m)
{
    ...
    /**/
    arr_sub(arr_available, req_vec, m);
    arr_add(mat_alloc[task_i], req_vec, m);
    ...
    if (/**/!state_safe(arr_available, mat_alloc, mat_need, n, m))
    {
        ...
        /**/
        arr_add(arr_available, req_vec, m);
        arr_sub(mat_alloc[task_i], req_vec, m);
        arr_add(mat_need[task_i], req_vec, m);
